...
#define ASM64(name) void __attribute((naked)) name(void)
...
template<typename T> __forceinline void InjectHook64(uintptr_t address, T hook)
{
	Unprotect_internal((void *)address, HOOK_SIZE + 8);
	*(unsigned char *)address = 0x49;
	*(unsigned char *)(address + 1) = 0xBA;
	
	*(unsigned char *)(address + 10) = 0x41;
	*(unsigned char *)(address + 11) = 0xFF;
	*(unsigned char *)(address + 12) = 0xE2;

	*(ptrdiff_t *)(address + 2) = (uintptr_t)(void *&)hook;
			
	Protect_internal((void *)address, HOOK_SIZE + 8);
}
